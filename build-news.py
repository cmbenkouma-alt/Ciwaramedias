...
try:
    ...
    for i,node in enumerate(root.findall('.//item')[:7]):
        title=node.findtext('title',''); link=node.findtext('link',''); desc=node.findtext('description','') or ''; pub=node.findtext('pubDate','') or ''
        img=''
        for c in node:
            tag=c.tag.split('}')[-1]
    ...
    print(f'RSS generation skipped: {e}')
    if not OUT.exists(): OUT.write_text(json.dumps({'items':[]},ensure_ascii=False),encoding='utf-8')
